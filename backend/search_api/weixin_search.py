...
def extract_wexin_datetime(publish_time: str):
    """
    "document.write(timeConvert('1756882346'))"
    Args:
        publish_time:
    Returns:
    """
    # 取出第一个 >=10 位的数字（常见时间戳长度）
    m = re.search(r'(\d{10,13})', publish_time)
    if not m:
        raise ValueError("字符串里没找到时间戳数字")
    ...
    new_publish_time = dt_shanghai.strftime("%Y-%m-%d %H:%M:%S")
...
def get_wechat_article(keyword: str, number=10):
    """
    获取前10篇文章
    """
    ...
    for every_result in results:
        ...
        url = get_real_url(sougou_link)
        # referer：请求来源
        status, content = get_article_content(url, referer=sougou_link)
        if not status:
            continue
        publish_time = every_result["publish_time"]
        if publish_time.startswith("document"):
            # 很有可能是这样的格式： "document.write(timeConvert('1756882346'))"
            publish_time = extract_wexin_datetime(publish_time)
        article = {
...
if __name__ == '__main__':
    result = get_wechat_article(keyword="吉利汽车",number=2)
    print(result)
