...
from .sub_agents.writer_agents.agent import writer_generator_loop_agent
from .utils import parse_outline_markdown_to_json  # 复用你已有的解析函数
...
def before_agent_callback(callback_context: CallbackContext):
    ...
    try:
        outline_parts, references = parse_outline_markdown_to_json(md_content)
        print(f"成功解析大纲为：{outline_parts}")
        print(f"成功解析refrences：{references}")
    except Exception:
        # 真解析出异常：直接告诉用户不合法并短路
        return types.Content(
            role="model",
    ...
    state["outline_parts"] = outline_parts
    # 来自大纲中的引用内容
    state["outline_source_references"] = references
    state["title"] = outline_parts.get("title")
    state["sections"] = outline_parts.get("sections")
    state["abstract"] = outline_parts.get("abstract")
    # 1代表的是abstract摘要，作为1块
    state["parts_plan_num"] = len(outline_parts.get("sections")) + 1
    state["markdown"] = md_content
    # 返回 None 继续执行后续 Agent: writer_generator_loop_agent
    return None
...
root_agent = SequentialAgent(
    ...
    sub_agents=[writer_generator_loop_agent],
