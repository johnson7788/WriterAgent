...
import { BookOpen, Edit3, ArrowRight, RotateCcw, ArrowLeft, Brain, ChevronDown, ChevronRight } from "lucide-react";
...
import { generateOutlineStream } from "~/core/api/review";
...
export default function OutlinePage() {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (chunkQueue.current.length > 0) {
        ...
        appendOutlineContent(chunkToAppend);
        chunkQueue.current = [];
      }
    }, 200);
      ...
      try {
        ...
        for await (const event of stream) {
          ...
          if (event_data.type === "text") {
            chunkQueue.current.push(event_data.text);
            fullContentRef.current += event_data.text;
  ...
  **Function Response:**
  ${JSON.stringify(
        ...
        if (isMounted) {
          ...
          setOutlineContent(fullContentRef.current);
  ...
  const handleRegenerate = () => {
    ...
    setReferences({}); // Clear references
    setThoughtProcess("");
    // Trigger regeneration by creating a new effect
  ...
  **Function Response:**
  ${JSON.stringify(
        ...
        console.error("Error regenerating outline:", e);
      } finally {
        if (chunkQueue.current.length > 0) {
          const remainingChunk = chunkQueue.current.join("");
          appendOutlineContent(remainingChunk);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        ...
        <main className="container mx-auto px-4 py-8">
          <ProgressIndicator currentStep="outline" />
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
            ...
            <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-600">
              ...
              <div className="text-blue-900">{topic}</div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto py-8">
          <div className="flex justify-end items-center space-x-4">
            ...
            <Button
              ...
              disabled={!outlineContent.trim() || isConfirming}
            ...
            >
              {isConfirming ? "正在处理..." : <>确认大纲，撰写文章 <ArrowRight className="ml-2 h-5 w-5" /></>}
            </Button>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 h-full">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-xl font-semibold text-gray-900">编辑大纲内容</h2>
              </div>
              <div className="border-2 border-gray-200 rounded-lg overflow-hidden">
                <ReportEditor content={outlineContent} onMarkdownChange={setOutlineContent} />
              </div>
            </div>
          </div>
