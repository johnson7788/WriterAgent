...
export const useStore = create<{
  ...
  generationStep: 'FORM' | 'GENERATING_OUTLINE' | 'CONFIRM_OUTLINE' | 'GENERATING_REVIEW' | 'EDIT_REVIEW';
  topic: string;
  language: string;
  ...
  // For review generation
  setGenerationStep: (step: 'FORM' | 'GENERATING_OUTLINE' | 'CONFIRM_OUTLINE' | 'GENERATING_REVIEW' | 'EDIT_REVIEW') => void;
  setTopic: (topic: string) => void;
  setLanguage: (language: string) => void;
  setOutlineContent: (content: string) => void;
  appendOutlineContent: (chunk: string) => void;
  setReviewContent: (content: string) => void;
  appendReviewContent: (chunk: string) => void;
}>((set) => ({
  responding: false,
  threadId: THREAD_ID,
  messageIds: [],
  ...
  appendOutlineContent: (chunk) => set((state) => ({ outlineContent: state.outlineContent + chunk })),
  setReviewContent: (content) => set({ reviewContent: content }),
  appendReviewContent: (chunk) => set((state) => ({ reviewContent: state.reviewContent + chunk })),
}));
